...
SED_CMD='s/\berror: [[:digit:]]\+:[[:digit:]]\+\b://g'

if [[ -s "$EXPECTED_PREFIX.stderr" ]]; then
    diff <(sed "$SED_CMD" $ACTUAL_PREFIX.stderr) <(sed "$SED_CMD" $EXPECTED_PREFIX.stderr)
else
    diff <(sed "$SED_CMD" $ACTUAL_PREFIX.stdout) <(sed "$SED_CMD" $EXPECTED_PREFIX.stdout)
    diff <(sed "$SED_CMD" $ACTUAL_PREFIX.stderr) <(sed "$SED_CMD" $EXPECTED_PREFIX.stderr)
fi
